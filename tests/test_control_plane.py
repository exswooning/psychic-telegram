...
class TestSignupIsClosedOnceThereIsAnAccount:
    """Signing up mints a session directly, so an open form is not merely a
    way to create a row -- it is a way in, bypassing the sign-in check."""

    def test_a_signup_is_refused_once_an_account_exists(self, cp, monkeypatch):
        first = cp.post("/api/v2/auth/signup",
                        json={"email": "first@example.com",
                              "password": "hunter22222", "name": "First"})
        assert first.status_code == 200, first.text
        cp.post("/api/v2/auth/logout")
        # conftest opens signup for the whole suite (most fixtures build
        # their accounts through this endpoint); drop it to see the
        # production default.
        monkeypatch.delenv("BITPORT_SIGNUP_OPEN", raising=False)
        second = cp.post("/api/v2/auth/signup",
                         json={"email": "second@example.com",
                               "password": "hunter22222", "name": "Second"})
        assert second.status_code == 403
        assert "not open for signup" in second.json()["detail"]

    def test_it_reopens_for_a_deployment_that_wants_self_service(self, cp):
        """BITPORT_SIGNUP_OPEN, which conftest sets for the suite."""
        cp.post("/api/v2/auth/signup",
                json={"email": "selfserve1@example.com",
                      "password": "hunter22222", "name": "One"})
        cp.post("/api/v2/auth/logout")
        r = cp.post("/api/v2/auth/signup",
