...
import {
  Alert, Box, Button, Divider, FormControlLabel, Radio, RadioGroup,
  Stack, Typography,
} from '@mui/material'
...
}> = ({ domain, adminEmail, adminPassword }) => {
  const [act, setAct] = useState<'wipe' | 'delete_users' | null>(null)
  const [note, setNote] = useState('')
  // Which slot this tenant occupies -- asked, never assumed.
  //
  // The wizard used to treat every domain it was given as the SOURCE. Live,
  // somebody set up their destination tenant through it and the source row
  // was overwritten with the target's domain and admin. That is not merely
  // a wrong row: seed_argv's typed-domain gate compares against
  // source_domain, so the target's name sitting in that column makes typing
  // the target's name PASS the check meant to stop a seed being aimed at
  // production. accounts_auth now refuses the collision outright; this is
  // the half that stops anyone reaching for it.
  const [side, setSide] = useState<'source' | 'target' | ''>('')
  ...
  return (
    <Box>
      ...
      <Typography variant="subtitle2" sx={{ fontWeight: 700, mb: 0.5 }}>
        Which side is {domain}?
      </Typography>
      <Typography variant="body2" color="text.secondary" sx={{ mb: 1.5 }}>
        A migration reads one tenant and writes the other, and the two are
        credentialed differently. This decides which.
      </Typography>
      <RadioGroup row value={side} sx={{ mb: 3 }}
                  onChange={(e) => setSide(e.target.value as 'source' | 'target')}>
        <FormControlLabel value="source" label="Migrating FROM it (source)"
          control={<Radio inputProps={{ 'data-testid': 'side-source' } as never} />} />
        <FormControlLabel value="target" label="Migrating INTO it (destination)"
          control={<Radio inputProps={{ 'data-testid': 'side-target' } as never} />} />
      </RadioGroup>

      {!side && (
        <Alert severity="warning" data-testid="side-unset">
          Pick a side before setting this tenant up. Guessing is how a
          destination tenant ends up recorded as a source, which disarms the
          check that stops a seed being aimed at it.
        </Alert>
      )}
      ...
      <Stack direction="row" spacing={1.5} sx={{ flexWrap: 'wrap', gap: 1 }}>
        <Button size="small" color="warning" variant="outlined" disabled={!side}
                startIcon={<WipeIcon />} data-testid="later-wipe"
                onClick={() => setAct('wipe')}>
          Wipe data
        </Button>
        <Button size="small" color="error" variant="outlined" disabled={!side}
      ...
      <TenantActionDialog
        ...
        onConfirm={async (password) => {
          const r = await removeTenantSetup(side as 'source' | 'target',
                                            domain, password, act!)
