...
import { NodeOs, codeCommand, joinCommand, removeCommand } from './AddNodeWizard.utils'
...
export const AddNodeWizard: React.FC<{
  ...
  // Read inside the mount effect via a ref, not a dependency: this must
  // fetch once on mount, and a parent passing a fresh onAccountChange
  // closure every render (the normal case for an inline prop) would
  // otherwise re-run the whole fetchMe() call on every one of the
  // parent's re-renders instead of once.
  ...
  useEffect(() => {
    fetchMe()
      .then((me) => {
        ...
        setAccounts([mine])
        setAccount(mine.id); onAccountChangeRef.current?.(mine.id)
