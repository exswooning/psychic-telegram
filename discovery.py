...
def scan_user(auth, db: MigrationDB, settings: Settings,
    ...
    # Depth over FOLDERS only. The field is named max_depth and the warning
    # below talks about folder depth; including files inflated it by one and
    # made the >20 alarm fire on trees that were fine.
    max_depth = max(
        (d for iid, d in depths.items()
         if items.get(iid, {}).get("mimeType") == FOLDER_MIME),
        default=0,
    )
