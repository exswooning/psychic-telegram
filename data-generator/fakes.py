...
class _DriveFiles:
    ...
    def _create(self, body: dict, media_body=None, fields: str = "",
        ...
        # Real Drive accepts the literal alias "root" as a parent and resolves
        # it to the user's actual root folder ID. Mirroring that here matters:
        # without it, anything created with parents=["root"] is orphaned in the
        # fake but perfectly fine in production — a divergence that would hide
        # real bugs rather than expose them.
        parents = [
            self.s.root_id if p == "root" else p
        ...
        elif str(meta.get("mimeType", "")).startswith(
            "application/vnd.google-apps."
        ) and meta["mimeType"] != FOLDER_MIME:
            # Creating a native file with no media body yields an *empty* Doc,
            # Sheet or Slides deck. Real Drive still exports it happily, so the
            # fake must give it an export payload — otherwise every blank native
            # file looks like a migration failure that would not occur in
            # production.
            self.s.exports[fid] = b"empty-native-export"
