...
| 4. Sandbox tenants | DWD config, real API semantics, cross-user duplication | ~1 h setup | before first live run |
...
pytest                       # 114 tests, ~7 seconds, no network
...
> **If you already have two tenants, go straight to `SANDBOX.md`** — it is the
> executable version of this section, with `tools/seed_sandbox.py` to build the
> corpus and `tools/rehearsal.py` to run the eight acceptance phases.
